...
func Initialize(ctx context.Context, params common.ExtensionInitializationMessage) (*RuntimeContext, error) {
	...
	case ExchangeRateProvider:
		...
		case CMCExchangeRateProviderKind:
			...
			if providerConfig.Slug == "" || providerConfig.Slug == "mavryk" {
	...
	if _, err := mavryk.ParseAddress(config.Token.Contract); err != nil {
	...
	result.MinimumTokenAmount = mavryk.NewZ(int64(config.MinimumTokenAmount * math.Pow10(config.Token.Decimals)))
