...
func TestCollectTransactionFees(t *testing.T) {
	...
	for i, v := range result.StageData.AccumulatedPayouts {
		assert.LessOrEqual(v.GetAmount().Int64()-constants.TEST_MUMAV_DEVIATION_TOLERANCE, payoutRecipes[i].GetAmount().Int64()-collector.GetExpectedTxCosts())
		assert.GreaterOrEqual(v.GetAmount().Int64()+constants.TEST_MUMAV_DEVIATION_TOLERANCE, payoutRecipes[i].GetAmount().Int64()-collector.GetExpectedTxCosts())
