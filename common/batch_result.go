...
func NewFailedBatchResultWithOpHash(Payouts []*AccumulatedPayoutRecipe, opHash mavryk.OpHash, err error) *BatchResult {
	result := NewFailedBatchResult(Payouts, err)
	result.OpHash = opHash
	return result
}

func NewSuccessBatchResult(payouts []*AccumulatedPayoutRecipe, opHash mavryk.OpHash) *BatchResult {
	return &BatchResult{
		Payouts:   payouts,
		OpHash:    opHash,
		IsSuccess: true,
...
func (br *BatchResult) ToIndividualReports() []PayoutReport {
	...
	for _, payout := range br.Payouts {
		for _, acc := range payout.Recipes {
			note := acc.Note
			if !br.IsSuccess {
				note = br.Err.Error()
			}
