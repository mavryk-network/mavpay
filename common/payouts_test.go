...
func TestPayoutRecipeToAccumulatedIdentifierMatches(t *testing.T) {
	...
	payoutRecipe := &PayoutRecipe{
		...
		FATokenId:        mavryk.NewZ(1),
		FAContract:       getRandomAddress(),
		FAAlias:          "aaa",
		FADecimals:       2,
		DelegatedBalance: mavryk.NewZ(1000000),
		StakedBalance:    mavryk.NewZ(2000000),
		Amount:           mavryk.NewZ(3000000),
		FeeRate:          5,
		Fee:              mavryk.NewZ(4000000),
		TxFee:            10,
		Note:             "aaa",
		IsValid:          false,
	}
...
func Test_AccumulatedAddRecipeSame(t *testing.T) {
	...
	payoutRecipe2 := &PayoutRecipe{
		...
		FAAlias:          "aaa",
		FADecimals:       2,
		DelegatedBalance: mavryk.NewZ(2000000),
		StakedBalance:    mavryk.NewZ(3000000),
		Amount:           mavryk.NewZ(4000000),
		FeeRate:          5,
		Fee:              mavryk.NewZ(5000000),
		TxFee:            10,
		Note:             "aaa",
		IsValid:          false,
	}
...
func TestAddTxFee(t *testing.T) {
	...
	assert.NoError(err)
	assert.Len(accumulated.Recipes, 2)
	assert.Equal(accumulated.GetAmount(), mavryk.NewZ(7000000))
	...
	assert.Equal(accumulated.GetAmount(), mavryk.NewZ(7000000).Sub(mavryk.NewZ(2000)))
	assert.Equal(accumulated.GetTxFee(), int64(3000))

	accumulated.AddTxFee64(7000000, true)
	assert.Equal(accumulated.GetAmount(), mavryk.Zero)
}
